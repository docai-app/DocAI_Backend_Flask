...
def add_text_and_convert(
    image_path,
    text,
    output_filename,
    page_number,
    image_size=(1360, 1024),
):  # 1174 height after adding white space
    ...
    mask = cv2.rectangle(
        mask, (0, 0), (img.shape[1], int(img.shape[0] * 0.95)), 255, cv2.FILLED
    )
...
def build_title_page(image_path, text, output_filename, image_size=(1360, 1174)):
    ...
    line_height = (
        style.leading
    )  # The leading property gives the height of a line of text in the given style
    padding = 50
